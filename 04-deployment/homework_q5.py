...
def run(year, month):
    ...
    input_file_url = f"https://d37ci6vzurychx.cloudfront.net/trip-data/yellow_tripdata_{year:04d}-{month:02d}.parquet"  # noqa: E501
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Скрипт для прогнозирования продолжительности поездок на такси."
    )
    parser.add_argument(
        "--year", type=int, required=True, help="Год для обработки (например, 2023)"
    )
    parser.add_argument(
        "--month",
        type=int,
        required=True,
        help="Месяц для обработки (например, 4 для апреля)",
    )
