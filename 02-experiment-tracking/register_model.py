...
# mlflow.sklearn.autolog() # Автологирование здесь, вероятно, не нужно,
# так как мы логируем метрики и модель вручную.
...
def train_and_log_model(data_path, params, experiment_id_for_new_run):
    ...
    with mlflow.start_run(
        experiment_id=experiment_id_for_new_run, nested=True
    ) as run:  # nested=True если вызывается из другого run
        run_id = run.info.run_id
        mlflow.set_tag(
            "source_hpo_run_params", str(params)
        )  # Сохраняем исходные HPO параметры как тег
        ...
        print(
            f"Run ID {run_id}: Trained with params {new_params}, Test RMSE: {test_rmse}"
        )
...
def run_register_model(data_path: str, top_n: int):
    ...
    try:
        ...
        if hpo_experiment is None:
            print(
                f"ERROR: HPO Experiment '{HPO_EXPERIMENT_NAME}' not found on server {MLFLOW_TRACKING_URI}."  # noqa: E501
            )
            return
        hpo_experiment_id = hpo_experiment.experiment_id
        print(
            f"Found HPO experiment '{HPO_EXPERIMENT_NAME}' with ID: {hpo_experiment_id}"
        )
    except Exception as e:
        print(
            f"ERROR: Could not get HPO experiment '{HPO_EXPERIMENT_NAME}'. Exception: {e}"
        )
        return
    ...
    try:
        ...
        if best_models_experiment is None:
            print(
                f"Experiment '{EXPERIMENT_NAME}' does not exist. Creating a new experiment."
            )
            best_models_experiment_id = client.create_experiment(EXPERIMENT_NAME)
            print(
                f"Created new experiment '{EXPERIMENT_NAME}' with ID: {best_models_experiment_id}"
            )
        else:
            best_models_experiment_id = best_models_experiment.experiment_id
            print(
                f"Found existing experiment '{EXPERIMENT_NAME}' with ID: {best_models_experiment_id}"  # noqa: E501
            )
        mlflow.set_experiment(experiment_id=best_models_experiment_id)
    except Exception as e:
        print(
            f"ERROR: Could not get/create experiment '{EXPERIMENT_NAME}'. Exception: {e}"
        )
        return
    ...
    if not runs:
        print(
            f"WARNING: No runs found in HPO experiment '{HPO_EXPERIMENT_NAME}' (ID: {hpo_experiment_id}). Cannot proceed."  # noqa: E501
        )
        return
        ...
        evaluated_runs.append(
            {
                "run_id": new_run_id,
                "test_rmse": test_rmse,
                "source_hpo_params": run.data.params,
            }
        )

    if not evaluated_runs:
        print(
            "ERROR: No models were evaluated from HPO runs, something went wrong in the loop."
        )
        return
    ...
    print(
        f"\nBest model for registration is from run_id: {best_run_id} with Test RMSE: {lowest_test_rmse}"  # noqa: E501
    )
    print(
        f"Source HPO params for best model: {best_evaluated_run['source_hpo_params']}"
    )
